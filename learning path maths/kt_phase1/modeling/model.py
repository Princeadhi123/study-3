"""Shared Transformer-based KT model for all three variants.
...
Architecture (SAKT/DKT-style, causal self-attention):
  interaction_i = embed(skill_{i-1}, item_{i-1}, correct_{i-1}, rt_{i-1},
                         attempt_{i-1}, content_{i-1})           (i=1 uses a
                         learned start token instead of position 0)
  h_1..h_T = CausalTransformerEncoder(interaction_1..interaction_T)
  query_i  = embed(skill_i, item_i, content_i)          -- no correctness!
...
class KTTransformer(nn.Module):
    ...
    def _content_vec(self, content_idx, content_vectors):
        if not self.use_content or content_vectors is None:
            return None
        vecs = content_vectors[content_idx]  # (B, T, content_dim), frozen lookup
        return self.content_proj(vecs)
    ...
    def forward(self, batch, content_vectors=None):
        ...
        if self.use_content:
            ...
            prev_content = self._content_vec(prev_content_idx, content_vectors)
        ...
        if self.use_content:
            query_parts.append(self._content_vec(content_idx, content_vectors))
